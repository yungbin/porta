      ...
      <router-link v-for="porta in paginatedPortas" :key="porta.ID"
        ...
        <article class="porta-card">
          ...
          <div class="porta-info">
            ...
            <P><i class="bi bi-balloon-heart"></i> {{ porta.LIKES }}</P>
            ...
            <div class="porta-meta">
              ...
              <span class="meta-item"><i class="fas fa-clock"></i> 평점 {{ porta.RATING }}</span>
            </div>
          </div>
        </article>
      </router-link>
...
export default {
  ...
  created() {
    this.fetchNewLinkList();
  },
  methods: {
    fetchNewLinkList() {
      axios.get('api/porta/newList')
        .then(response => {
