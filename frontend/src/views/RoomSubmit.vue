<template>
    <div class="room-create">
      ...
      <form @submit.prevent="saveRoom" class="create-form">
        <div class="form-row">
          ...
          <div class="form-group">
            <label for="room_nm">장소</label>
            <input id="room_nm" v-model="room.ROOM_NM" required>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label for="theme_nm">SITE</label>
            <input id="theme_nm" v-model="room.SITE" required>
          </div>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="radio-group">
              <label class="radio-label">
                <input type="radio" v-model="room.RCMD" value="Y" id="rcmdY">
                <span class="radio-custom"></span>
                Y
              </label>
              <label class="radio-label">
                <input type="radio" v-model="room.RCMD" value="N" id="rcmdN">
                <span class="radio-custom"></span>
                N
              </label>
            </div>
            ...
            <div class="radio-group">
              <label class="radio-label">
                <input type="radio" v-model="room.NEW_THEME" value="Y" id="newThemeY">
                <span class="radio-custom"></span>
                Y
              </label>
              <label class="radio-label">
                <input type="radio" v-model="room.NEW_THEME" value="N" id="newThemeN">
                <span class="radio-custom"></span>
                N
              </label>
            </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input id="price" v-model.number="room.PRICE" required>
            </div>
        </div>
  ...
  export default {
    ...
    setup() {
      ...
      const saveRoom = async () => {
        try {
          room.value.IMG_PATH = `/poster/${room.value.THEME_NM}.jpg`;
          const roomData = { ...room.value, userId: user.value.id }; 
          await axios.post(`/api/room/submit`, roomData);
...
.radio-label input[type="radio"]:checked ~ .radio-custom {
  ...
  font-size: 16px;
  transition: border-color 0.3s;
  box-sizing: border-box; /* 패딩과 보더를 폭에 포함시킴 */
